...
| `alphaproof` (this branch) | [`ChristianZ97/satp-policy-v2-alphaproof`](https://huggingface.co/ChristianZ97/satp-policy-v2-alphaproof) `a654c92d` | [`ChristianZ97/minif2f-satp-alphaproof`](https://huggingface.co/datasets/ChristianZ97/minif2f-satp-alphaproof) `81a8abfa` | standalone `v4.26.0` (below) |
...
- Checkpoint and inference code are downloaded at the pinned HF revision by
  ...
  On this branch the eval-dataset revision (`81a8abfa`) is additionally
  pinned inside the HF repo's `infer.py`.
...
```bash
git clone --branch alphaproof https://github.com/ChristianZ97/LeanSATP.git
cd LeanSATP
./setup.sh
```

`./setup.sh` builds the bundled Lean environment under `deps/`, wires the local
LeanCopilot/CTranslate2 paths into Lake, and downloads the pinned alphaproof
policy assets into `cache_v2/`. No shell export is needed for normal Lean use.
...
[`ChristianZ97/satp-policy-v2-alphaproof`](https://huggingface.co/ChristianZ97/satp-policy-v2-alphaproof)
